(function ($, global, undef) {
        ...
        function loadShowSettings(activator, context) {
            return $.Deferred(function (dfd) {
                context.onShow(activator, function (settings) {
                    dfd.resolve($.extend({}, getShowSettings(), settings || {}));
                });
            }).promise();
        }
    ...
    function initSearchPopup(context) {
        ...
        function getCurrentSearchFilters(context) {
            ...
            var selectedLanguage = context.searchFilter.find('a.language.selected');
            var filters = {
                placeKey: selectedPlace.data('key') || 'anywhere',
                placeValue: selectedPlace.data('value') || 'anywhere',
                type: selectedType.data('key'),
                query: $.trim(context.searchInput.val()),
                language: selectedLanguage.data('key') || 'English',
        ...
        function loadSearchPlaces(context) {
            ...
            if (context.searchPlaceGroupContainerId) {
                places.push({
                    key: 'group',
                    value: context.searchPlaceGroupContainerId,
        ...
        function loadSearchLanguages(context) {
            var languages = [];
            ...
            context.searchLanguages = languages;
        }
        ...
        function loadSearchFilters(context, scope) {
            ...
            if (scope.key == 'anywhere' || scope.key == 'group') {
                ...
                filters.push({
                    key: 'community',
                    name: context.searchFilterCommunityName,
                    searchParameter: '',
                    searchValue: ''
                });
                ...
                filters.push({
                    key: 'Documentation',
                    name: context.searchFilterDocumentName,
                    searchParameter: '',
                    searchValue: ''
                });
                ...
                filters.push({
                    key: 'kb',
                    name: context.searchFilterArticleName,
                    searchParameter: '',
                    searchValue: ''
                });
            }
        ...
        function setDefaultAdvancedSearchUrl(context) {
            ...
            var effectiveSearchFilter = getCurrentSearchFilters(context);

            var query = $.trim(effectiveSearchFilter.query);
            if (query && query.length > 0) {
                var params = { q: query };

                var group = '';
                if (effectiveSearchFilter.searchParameter && effectiveSearchFilter.searchValue) {
                    params[effectiveSearchFilter.searchParameter] = effectiveSearchFilter.searchValue;
                }
                if (effectiveSearchFilter.placeKey == 'group' || effectiveSearchFilter.placeKey == 'application') {
                    group = context.searchPlaceGroupId;
                }

                var filters = {
                    serplanguages: effectiveSearchFilter.language,
                    serpcategory: effectiveSearchFilter.type,
                    serpgroup: group
                };

                context.currentAdvancedSearchUrl = context.searchAdvancedUrl
					.replace(/\{0\}/gi, $.param(params))
                    .replace(/\{1\}/gi, $.param(filters))
					.replace(/\+/gi, '%20')
					.replace(/'/gi, '%27');
        ...
        function buildSearchDataFromEffectiveQuery(pageIndex, effectiveSearchFilter) {
            return {
                ...
                w_productValue: context.productPlaceGroupId,
                w_type: effectiveSearchFilter.type,
                w_language: effectiveSearchFilter.language
            };
        }
                ...
                if (filter && filter.query) {
                    ...
                    $.telligent.evolution.get({
                        ...
                        success: function (response) {
                            ...
                            if ($.telligent.evolution.url.serializeQuery(currentQueryData) !=$.telligent.evolution.url.serializeQuery(queryData)) {
                                complete(null);
                                return;
                            }
            ...
            onBuildFilter: function (filtered) {
                ...
                var filterTemplate = $.telligent.evolution.template.compile(context.searchFilterTemplate),
					filterTemplateData = {
					    ...
					    languages: context.searchLanguages
					};
                ...
                context.searchFilter.find('a.language:first').addClass('selected');
    ...
    function initSiteNavigation(context) {
        context.siteNavigationList = HeaderList({
            ...
            endlessScroll: true,
            titleCount: false,
           // cssClass: ('site ' + (context.siteNavigationType === 'my_groups' ? 'group' : context.siteNavigationType) + ' ' + (context.siteNavigationType == 'custom' ? '' : '')),
            cssClass: ('site ' + (context.siteNavigationType === 'my_groups' ? 'group' : context.siteNavigationType) + ' ' + ('')),
            wrapper: context.banner,
    ...
    function initKeyboardEvents(context) {
        ...
        $(context.searchInput).on('keyup', function (e) {
            document.cookie = "searchcache=searchtext=" + context.searchInput.val() + ";path=/;max-age=300";
        });
    }
